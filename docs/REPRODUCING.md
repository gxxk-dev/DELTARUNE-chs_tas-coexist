...
当前构建 ID 为 `pc-v0.0.247-f3437be-260710`。schema 2 主机器锁
[`versions/pc-v0.0.247-f3437be-260710.json`](../versions/pc-v0.0.247-f3437be-260710.json)
是基础共存版版本、大小、SHA256、来源路径和输出集合的事实源；可选 Ralsei 本地变体的
资产身份、导入器和五章输出由
...
| Samuton Ralsei 肖像 | 仅用户显式提供的本地 7z；可选 | `ralsei-portraits-samuton-v1` 清单锁定身份 |
...
10. 校验基础共存版的全部固定哈希、音频属性、Chromaprint 内容指纹、关键反编译代码与
    输出文件集合。
11. 若显式传入 `--ralsei-archive FILE`，再校验本地归档身份，仅提取清单锁定的 297 个
    `Replace/*` PNG，并在汉化、全部热修和基础版验证之后导入 Chapter 1-5。其他归档成员
    和归档自带的 Windows UndertaleModTool 均不使用，main `data.win` 保持基础共存版不变；
    导入后重新校验五章变体哈希和完整共存规则。
12. 校验本次请求对应的最终输出；
    基础版写入 schema 2 `build-info.json`，本地素材变体写入带 `variant` 身份的 schema 3。
    若指定 `--patchset-dir`，此时创建并回放验证本地补丁集。全部成功后才原子替换目标
    `output/`。
...
Ralsei 肖像归档既不会下载，也不会进入缓存。在线或离线构建该变体前，用户都必须观看
[BV1mNMv6NEiY](https://www.bilibili.com/video/BV1mNMv6NEiY)，按视频中的作者说明自行合法下载
`Samuton.Ver Ralsei Portrait Texture Replacement Pack.7z`，并显式传入其本地路径：
...
归档路径不会写入构建元数据；`build-info.json` 只标识素材 ID、大小和 SHA256。仓库不
下载、不嵌入、不分发原始归档、PNG、派生 `data.win` 或派生 BPS。
...
- `manifest.json`：完整来源、输入/输出/BPS 哈希、文件大小，以及五条
  `derived_copies` 形式的 `data_keucher.win` 复制规则；基础版使用 schema 2，Ralsei 变体
  使用包含素材 `variant` 身份的 schema 3；
- `README.txt`：独立回放顺序。

每份 BPS 创建后都会立即应用到对应干净输入，并检查结果 SHA256。补丁集由游戏本体与
第三方资产派生，只用于持有者本机归档或研究，不得加入 Git、Release 或其他分发物。

Ralsei 变体无需第二次手工处理；把 `--ralsei-archive FILE` 与 `--patchset-dir` 放在同一
条 `build.sh` 命令中，即可一次生成统一的 Keucher + CHS + Ralsei 输出及一份对应的本地
补丁集。该补丁集仍由 main 与五章的六份 BPS 组成，并以变体哈希完成独立回放验证；包含
版权素材派生结果，因此同样不得分发。

## 输出验证边界

基础共存版的跨环境固定输出包括六个最终 `data.win` 和 21 个静态外置文件，后者为语言
JSON 与视频；它们全部按主机器锁校验字节数（适用时）与 SHA256。Ralsei 变体的 main
`data.win` 和外置文件与基础版相同，Chapter 1-5 则按本地素材清单锁定的变体 bytes/SHA256
验证。源 WAV 同样锁定 bytes/SHA256。两份
...
`build-info.json` 绑定构建 ID、基础版或本地素材变体、上游/适配器/UTMT provenance、
构建环境版本、六个固定输出哈希，以及本机 OGG 的 bytes/SHA256；变体只记录素材 ID、
bytes/SHA256，不记录本机路径。安装器与补丁集只接受和这份声明一致的两份
...
状态。安装器从 `build-info.json` 自动识别基础 schema 2 或 Ralsei 变体 schema 3，并选用
对应的五章哈希；两种输出使用相同的 `apply` / `restore` 命令。安装后被其他程序修改的
文件不会被还原流程覆盖。
...
- 可选 Samuton 肖像必须最后覆盖 Chapter 1-5 的目标 sprite 帧；只读取 `Replace/*` 能避免
  把归档内无关媒体或第三方 Windows UTMT 混入受验证工具链。
